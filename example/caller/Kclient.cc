...
void send_request(int thread_id, std::atomic<int> &success_count, std::atomic<int> &fail_count,int requests_per_thread) {
    ...
    //Kuser::UserServiceRpc_Stub stub(new KrpcChannel(false));    //！bug：单参数时，stub不负责管理channel，导致内存泄漏(看user.pb.cc 1515行)

    //解决办法：1.手动管理  2.双参数让stub管理channel 3.智能指针
    //方法二的写法：Kuser::UserServiceRpc_Stub stub(new KrpcChannel(false),::PROTOBUF_NAMESPACE_ID::Service::STUB_OWNS_CHANNEL);
    //方法三：
    auto channel = std::make_unique<KrpcChannel>(false);        //make_unique()在C++14及以上才有
    Kuser::UserServiceRpc_Stub stub(channel.get());
    ...
    for (int i = 0; i < requests_per_thread; ++i) {
        ...
        if (controller.Failed()) {  // 如果调用失败
            LOG(ERROR) << controller.ErrorText();  // 打印错误信息
            
            fail_count++;  // 失败计数加 1
        } else {  // 如果调用成功
            if (int{} == response.result().errcode()) {  // 检查响应中的错误码
                LOG(INFO) << "rpc login response success:" << response.success() << std::endl;  // 打印成功信息
                success_count++;  // 成功计数加 1
            } else {  // 如果响应中有错误
                LOG(ERROR) << "rpc login response error : " << response.result().errmsg() << std::endl;  // 打印错误信息
                fail_count++;  // 失败计数加 1
            }
        }
    }
...
int main(int argc, char **argv) {
    ...
    const int requests_per_thread = 50; // 每个线程发 1 次请求
