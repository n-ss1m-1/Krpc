...
void KrpcProvider::Run() {
    ...
    for (auto &sp : service_map) {
        ...
        for (auto &mp : sp.second.method_map) {
            std::string method_path = service_path + "/" + mp.first;    //临时节点："/"+service_name+"/"+method_name
...
void KrpcProvider::OnMessage(const muduo::net::TcpConnectionPtr &conn, muduo::net::Buffer *buffer, muduo::Timestamp receive_time) {
    ...
    while (buffer->readableBytes() >= 4) {
        ...
        //分辨request的类型+返回正确类型的空对象
        google::protobuf::Message *request = service->GetRequestPrototype(method).New();
        if (!request->ParseFromString(args_str)) {              //填充request所需的参数
            LOG(ERROR) << "request parse error";
            return;
        }
        //分辨response的类型+返回正确类型的空对象
        google::protobuf::Message *response = service->GetResponsePrototype(method).New();

        //回调函数
        //将SendRpcResponse,conn, response绑定在一起
        //功能：把 response 序列化后通过网络发回客户端
        ...
        //protobuf生成的分发函数, 负责路由到你重写的那个Login虚函数(Kserver.cc中)                                                                                            
