...
//初始化/定义静态成员
std::mutex ZkClient::m_cache_mutex;
std::unordered_map<std::string,std::string> ZkClient::m_host_cache;   
...
void global_watcher(zhandle_t *zh, int type, int status, const char *path, void *watcherCtx) 
{
    ...
    if (type == ZOO_SESSION_EVENT) {  // 回调消息类型和会话相关的事件
        if (status == ZOO_CONNECTED_STATE) {  // ZooKeeper客户端和服务器连接成功
            ...
            //将唤醒操作放到解锁后(lock_guard离开作用域自动释放)
    ...
    {
        m_client->DeleteCache(path);                        //删除失效的缓存
    }
    else if (type == ZOO_CHANGED_EVENT)
    {
        std::string new_data = m_client->GetData(path);     //获取更新后的alue + 续上监听
        m_client->SetCache(path,new_data);                  //更新缓存
    }
}
/*！此处watcher实现的局限：
...
void ZkClient::Start() {
    if (m_zhandle != nullptr) {
        LOG(INFO) << "ZooKeeper already connected, skipping Start()";
        return;
    }
...
std::string ZkClient::GetData(const char *path) {
    ...
    // 获取指定节点的数据
    int flag = zoo_get(m_zhandle, path, 1, buf, &bufferlen, nullptr);       //1：注册节点watcher监听
    if (flag != ZOK) {  // 获取失败
...
bool ZkClient::HasCache(const std::string& path)
{
    std::lock_guard<std::mutex> lock(m_cache_mutex);
    auto it = m_host_cache.find(path);
    if(it == m_host_cache.end()) return false;
    return true;
}
std::string ZkClient::GetCache(const std::string& path)
{
    std::lock_guard<std::mutex> lock(m_cache_mutex);
    auto it = m_host_cache.find(path);
    return it->second;
}
void ZkClient::SetCache(const std::string& path,const std::string& value)    
{
    std::lock_guard<std::mutex> lock(m_cache_mutex);
    m_host_cache[path] = value;
}
void ZkClient::DeleteCache(const std::string& path)
{
    std::lock_guard<std::mutex> lock(m_cache_mutex);
    m_host_cache.erase(path);
}